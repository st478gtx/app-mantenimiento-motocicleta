...
function obtenerServiciosIniciales(detallesIniciales) {
  return detallesIniciales.map((detalle) => detalle.servicioId);
}

function obtenerRepuestosIniciales(detallesIniciales) {
  return detallesIniciales.flatMap(
    (detalle) =>
      detalle.repuestos?.map((repuesto) => repuesto.repuestoId) ?? [],
  );
}

function getRepuestosDeServicio(servicioId) {
  const relaciones = servicioRepuestos.filter(
    (relacion) => relacion.servicioId === servicioId,
  );

  const repuestoIds = relaciones.map((relacion) => relacion.repuestoId);

  return repuestos.filter((repuesto) => repuestoIds.includes(repuesto.id));
}
...
export default function ServicioRepuestoModal({
  ...
  const [listRepuestoId, setListRepuestoId] = useState(() => {
    const servicioInicial = servicios.find((servicio) =>
      obtenerServiciosIniciales(detallesIniciales).includes(servicio.id),
    );
    ...
    const serviciosDeZona = servicios
      .filter((servicio) => servicio.zonaId === servicioInicial.zonaId)
      .map((servicio) => servicio.id);

    return [
      ...new Set(
        servicioRepuestos
          .filter((item) => serviciosDeZona.includes(item.servicioId))
  ...
  const [selected, setSelected] = useState(() => {
    ...
    const zonaInicial = zonasMoto.find(
      (zona) => zona.id === servicioInicial.zonaId,
    );
  ...
  const [zona, setZona] = useState(() => {
    ...
    return zonasMoto.find((zona) => zona.id === servicioInicial.zonaId) ?? {};
  });

  const [servicioAgregado, setServicioAgregado] = useState(() =>
    obtenerServiciosIniciales(detallesIniciales),
  );

  const [repuestoAgregado, setRepuestoAgregado] = useState(() =>
    obtenerRepuestosIniciales(detallesIniciales),
  );
  ...
  function seleccionZona(event) {
    const nombreZona = event.target.value;

    setSelected(nombreZona);

    const zonaSeleccionada = zonasMoto.find(
      (item) => item.nombre === nombreZona,
    );
    ...
    cargarRepuestos(zonaSeleccionada.id);
  }
  ...
  function cargarRepuestos(zonaId) {
    const servicioZona = servicios
      .filter((servicio) => servicio.zonaId === zonaId)
      .map((servicio) => servicio.id);

    const repuestoFiltro = servicioRepuestos
      .filter((item) => servicioZona.includes(item.servicioId))
      .map((item) => item.repuestoId);

    setListRepuestoId([...new Set(repuestoFiltro)]);
  }
  ...
  const serviciosSeleccionados = servicios.filter((servicio) =>
    servicioAgregado.includes(servicio.id),
  );

  /*
   * Repuestos completos seleccionados.
   */
  const repuestosSeleccionados = repuestos.filter((repuesto) =>
    repuestoAgregado.includes(repuesto.id),
  );
  ...
  const totalServicios = serviciosSeleccionados.reduce(
    (total, servicio) => total + servicio.precioBase,
    0,
  );
  ...
  const totalRepuestos = repuestosSeleccionados.reduce(
    (total, repuesto) => total + repuesto.precioVenta,
    0,
  );
   ...
   */
  const serviciosDeZona = zona.id
    ? servicios.filter(
        (servicio) => servicio.zonaId === zona.id && servicio.estado,
      )
    : [];
   ...
   */
  const repuestosDeZona = repuestos.filter((repuesto) =>
    listRepuestoId.includes(repuesto.id),
  );
      ...
      <div className="orden-trabajo-main">
        ...
        <div className="orden-container">
          ...
          <div className="moto-container">
            ...
            <img src={hondaRevel} className="moto-image" alt="Motocicleta" />
          ...
          <div className="zona-servicio">
            <div>
              ...
              <div className="zona-descripcion">{zona.descripcion ?? ""}</div>

              <div className="zona-servicio-titulo">Servicios disponibles</div>

              {serviciosDeZona.length > 0 ? (
                serviciosDeZona.map((servicio) => (
                  <div className="zona-check" key={servicio.id}>
                    <input
                      type="checkbox"
                      className="checkbox-custom"
                      checked={servicioAgregado.includes(servicio.id)}
                      onChange={() => manejoCambioServicio(servicio.id)}
                    />
            ...
            <div className="zona-servicio-inferior">
              {repuestosDeZona.length > 0 ? (
                <div>
                  <div className="zona-servicio-titulo">
                    Repuestos disponibles
                  </div>

                  {repuestosDeZona.map((repuesto) => (
                    <div className="zona-check" key={repuesto.id}>
                      <input
                        type="checkbox"
                        className="checkbox-custom"
                        checked={repuestoAgregado.includes(repuesto.id)}
                        onChange={() => manejoCambioRepuesto(repuesto.id)}
                      />
            ...
            <div>
              <div className="seleccion-servicio-barra">
                ...
                <div className="total-pago-container">
                  Total a pagar
                  <span>{formatoMoneda(total, "PEN", "es-PE")}</span>
                </div>
              </div>

              <div className="breadcrumb-container">
                {serviciosSeleccionados.map((servicio) => (
                  <div className="breadcrumb-item" key={servicio.id}>
                    {servicio.nombre}

                    <button
                      type="button"
                      onClick={() => manejoCambioServicio(servicio.id)}
                    >
                      X
                    </button>
                  </div>
            ...
            <div>
              ...
              <div className="breadcrumb-container">
                {repuestosSeleccionados.map((repuesto) => (
                  <div className="breadcrumb-item" key={repuesto.id}>
                    {repuesto.nombre}

                    <button
                      type="button"
                      onClick={() => manejoCambioRepuesto(repuesto.id)}
                    >
                      X
                    </button>
                  </div>
